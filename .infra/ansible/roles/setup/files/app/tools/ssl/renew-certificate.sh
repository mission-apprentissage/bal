...
start_reverse_proxy() {
  bash /opt/bal/start-app.sh "$(git --git-dir=/opt/bal/repository/.git rev-parse --abbrev-ref HEAD)" \
    --no-deps reverse_proxy
}

stop_reverse_proxy() {
  bash /opt/bal/stop-app.sh bal_reverse_proxy --no-deps reverse_proxy
}
