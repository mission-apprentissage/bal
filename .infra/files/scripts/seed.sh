...
cat "$SEED_ARCHIVE" | docker compose -f "/opt/app/docker-compose.preview-system.yml" exec -iT mongodb mongorestore --archive --nsFrom="mna-bal.*" --nsTo="$TARGET_DB.*" --drop --gzip "mongodb://__system:{{vault[env_type].MNA_BAL_MONGODB_KEYFILE}}@localhost:27017/?authSource=local&directConnection=true"
