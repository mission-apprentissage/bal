...
export async function createMailingList({ name, delimiter, expiresInDays, file }: IImportParams, user: IUser): Promise<IMailingListV2> {
...
export async function updateMailingListParseSettings(id: ObjectId, { name, delimiter, expiresInDays }: Omit<IImportParams, "file">) {
