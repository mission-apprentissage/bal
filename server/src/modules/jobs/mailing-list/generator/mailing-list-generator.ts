...
export async function validateMailingListConfiguration(mailingList: IMailingListV2) {
  ...
  if (mailingList.config.output_columns.some((i) => i.input.type === "computed" && i.input.name === "WEBHOOK_LBA") && !mailingList.config.lba_columns) {
...
export async function updateMailingListConfiguration(id: IMailingListV2["_id"], config: IMailingListV2ConfigUpdateQuery): Promise<void> {
...
async function addBouncerData({ computedLines, signal }: { computedLines: IMailingListComputedDatum[]; signal: AbortSignal }) {
...
export async function generateMailingList(mailingList: IMailingListV2, job: IJobsSimple, signal: AbortSignal): Promise<void> {
  const [lastSource, lastImported] = await Promise.all([
    ...
    getDbCollection("mailingList.computed").findOne({ mailing_list_id: mailingList._id }, { sort: { line_number: -1 } }),
  ])
