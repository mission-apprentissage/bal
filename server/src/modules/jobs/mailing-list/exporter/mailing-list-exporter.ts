...
async function* buildLine(mailingList: IMailingListV2, signal: AbortSignal) {
  ...
  const maxByGroup = await getDbCollection("mailingList.computed")
    .aggregate<{ _id: string; count: number }>(
      [{ $match: filter }, { $sort: { mailing_list_id: 1, email: 1 } }, { $group: { _id: "$email", count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 1 }],
      { signal }
    )
    .toArray()

  const groupCount = await getDbCollection("mailingList.computed")
    .aggregate<{ count: number }>([{ $match: filter }, { $sort: { mailing_list_id: 1, email: 1 } }, { $group: { _id: "$email", count: { $sum: 1 } } }, { $count: "count" }], {
      signal,
    })
    .toArray()
  ...
  }>([{ $match: filter }, { $sort: { mailing_list_id: 1, email: 1 } }, { $group: { _id: "$email", data: { $push: "$data" } } }], { signal })
  ...
  const [empty, blacklisted, invalid] = await Promise.all([
    ...
    getDbCollection("mailingList.computed")
      .aggregate<{ count: number }>([{ $match: { mailing_list_id: mailingList._id, email_status: "blacklisted" } }, { $count: "count" }])
      .toArray(),
    getDbCollection("mailingList.computed")
...
export async function exportMailingList(mailingList: IMailingListV2, job: IJobsSimple, signal: AbortSignal): Promise<void> {
  await getDbCollection("mailingListsV2").updateOne(
    { _id: mailingList._id },
    {
      $set: {
