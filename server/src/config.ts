...
const config = {
  ...
  ovhStorageSupport: {
    username: env.get("MNA_BAL_OVH_STORAGE_USERNAME_SUPPORT").required().asString(),
    password: env.get("MNA_BAL_OVH_STORAGE_PASSWORD_SUPPORT").required().asString(),
    authURL: "https://auth.cloud.ovh.net/v3/auth",
    containerName: env.get("MNA_BAL_OVH_STORAGE_CONTAINER_NAME_SUPPORT").required().asString(),
    encryptionKey: env.get("MNA_BAL_OVH_STORAGE_ENCRYPTION_KEY_SUPPORT").required().asString(),
    uri: env.get("MNA_BAL_OVH_STORAGE_URI_SUPPORT").required().asString(),
  },
