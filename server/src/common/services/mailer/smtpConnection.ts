...
type IResponseCode = `${keyof RESPONSE_CODES}`
    ...
    while (cmd !== "QUIT") {
      switch (cmd) {
        ...
        case "RSET":
          ...
          break
        case "CONNECT":
          throw new Error("CONNECT command should not be used in this context")
        default:
...
function isVrfySupported(res: Response, cmd: SMTP_COMMAND, extensions: ELHO_RESULT["extensions"]): VRFY_RESULT | ERROR_RESULT {
...
export async function vrfy(connection: SMTPConnection, extensions: ELHO_RESULT["extensions"]): Promise<VRFY_RESULT | ERROR_RESULT> {
...
export async function vrfyWorkaround(connection: SMTPConnection, extensions: ELHO_RESULT["extensions"]): Promise<VRFY_RESULT | ERROR_RESULT> {
