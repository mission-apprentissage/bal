...
export function MailingListView(props: { id: string }) {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <Button
            ...
            title={canRemove ? "Supprimer la liste de diffusion" : "Impossible de supprimer la liste de diffusion tant qu'elle est en cours de traitement"}
          >
      ...
      >
        <Typography>Êtes-vous sûr de vouloir supprimer cette liste de diffusion ? Cette action est irréversible.</Typography>
        {!canRemove && <Alert className={fr.cx("fr-mt-2w")} severity="info" title="La liste de diffusion est en cours de traitement, elle ne peut pas être supprimée." />}

        {removeMutation.isError && <Alert title="Une erreur est survenue lors de la suppression" description={removeMutation.error.message} severity="error" />}
      </removeModal.Component>
    </>
  )
}
