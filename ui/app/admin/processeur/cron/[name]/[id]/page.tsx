...
export default function JobInstancePage({ params }: { params: Promise<{ name: string; id: string }> }) {
  ...
  return (
    <Box>
      <Breadcrumb pages={[PAGES.adminProcessor(), PAGES.adminProcessorCron(name), PAGES.adminProcessorCronTask({ name, id })]} />
      <Typography variant="h2" gutterBottom>
        {PAGES.adminProcessorCronTask({ name, id }).title}
      </Typography>
      <ProcessorStatusProvider>
        {(status) => <ProcessorStatusTaskComponent name={name} status={status} baseUrl={new URL(PAGES.adminProcessor().path, publicConfig.baseUrl).href} id={id} type="cron" />}
      </ProcessorStatusProvider>
    </Box>
  )
}
