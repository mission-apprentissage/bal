...
export default function JobInstancePage({ params }: { params: Promise<{ name: string; id: string }> }) {
  ...
  return (
    <Box>
      <Breadcrumb pages={[PAGES.adminProcessor(), PAGES.adminProcessorJob(name), PAGES.adminProcessorJobInstance({ name, id })]} />
      <Typography variant="h2" gutterBottom>
        {PAGES.adminProcessorJobInstance({ name, id }).title}
      </Typography>
      <ProcessorStatusProvider>
        {(status) => <ProcessorStatusTaskComponent name={name} status={status} baseUrl={new URL(PAGES.adminProcessor().path, publicConfig.baseUrl).href} id={id} type="job" />}
      </ProcessorStatusProvider>
    </Box>
  )
}
