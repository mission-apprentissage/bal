...
export const responsableLegalControl: CerfaControl[] = [
  {
  ...
  },
  {
    ...
    process: ({ values, cache }) => {
      if (shouldAskRepresentantLegal({ values })) {
        const cachedValueOrResetRequired = (value: any) => ({ value, reset: !value, required: true });
        return {
          cache,
          cascade: {
  ...
  },
  {
    ...
    process: ({ values, cache }) => {
      const cachedValueOrReset = (value: any) => ({ value, reset: !value });
      const cachedValueOrResetRequired = (value: any) => ({ value, reset: !value, required: true });
      ...
      if (memeAdresse !== true && memeAdresse !== false) {
        ...
        const createCascadeValue = (value: any) => ({ value, reset: true });
        return {
