...
const ProfilPage = () => {
  ...
  return (
    <>
      ...
      {apiKey && (
        <>
          <Typography gutterBottom>Ce jeton n'est visible qu'une fois, il est recommandé de le stocker dans un endroit sécurisé.</Typography>
          <Box my={2}>
      ...
      {!apiKey && (
        <Typography>
          {user.api_key_used_at ? <>{`Dernière utilisation le ${formatDate(user.api_key_used_at as unknown as string, "PPP à p")}`}</> : <>Ce jeton n'a pas encore été utilisé</>}
        </Typography>
      )}
